...
function handleSyllableLine(line, currentTime) {
  line.letterSpans.forEach(span => span.classList.remove('active', 'glow'));
  ...
  line.syllables.forEach((syllable, syllableIndex) => {
    ...
    if (wordDuration > 1) {
      console.log("yes")
      wordContainer.container.classList.add('glow');
    }
...
function handleSyllableLine(line, currentTime) {
  ...
  line.syllables.forEach((syllable, syllableIndex) => {
    ...
    wordLetters.forEach((letter, letterIndex) => {
      ...
      if (currentTime >= letterTiming) {
        if (wordDuration > 1) letter.classList.add("glow")
        letter.classList.add('active');
      }
    });
...
function updatePlayback() {
  ...
  previewData.forEach((line, index) => {
    ...
    if (currentTime < line.begin) {
      ...
      line.letterSpans.forEach(span => span.classList.remove('active', 'glow'));
      line.wordContainers.forEach(wordObj => wordObj.container.classList.remove('finished'));
      line.element.classList.remove('active', 'glow', 'finished');
    }
    else if (currentTime >= line.end + 0.4) {

      line.element.classList.remove('active', 'glow');
...
function stopPlayback() {
  ...
  isPlaying = false;
  document.getElementById('stopButton').classList.remove('active', 'glow');
  ...
  previewData.forEach(line => {
    ...
    line.sublyrics?.forEach(sub => {
      ...
      sub.letters.forEach(letter => letter.classList.remove('active', 'glow'));
    });
  });
